...
def pickRandomDirection(current_room, visited):
    chosen_direction = None

    if current_room.id not in visited:
    # if the room is a new room, mark all exits as ?
        exits = current_room.getExits()
        directions = {}
        for exit in exits:
            directions[exit] = '?'
            visited[current_room.id] = directions
            chosen_direction = random.choice(list(directions.keys()))
    else:
        #check if theres' an exit that is still ?
        directions = visited[current_room.id]
        other_possible_directions = []
        for direction in directions:
            if directions[direction] == '?':
                other_possible_directions.append(direction)
        if len(other_possible_directions) > 0:
            chosen_direction = random.choice(other_possible_directions)
    return chosen_direction

def get_to_deadend(current_room, visited):
    #current path
    path = []
    #pick a direction using the current_room's exits
    next_dir = pickRandomDirection(current_room, visited)
    while next_dir is not None:
        #appends each directions as a path
        path.append(next_dir)
        #player travels in the direction
        player.travel(next_dir)
        #set the new_room as the player's current room.
        new_room = player.currentRoom
        # save the new room in the current room's directions
        visited[current_room.id][next_dir] = new_room
        # if the new_room is not in visited, get all the exits.
        if new_room.id not in visited:
            #get all the exits.
            found_exits = new_room.getExits()
            #create a dictionary of direction
            directions = {}
            for direction in found_exits:
                #if direction is the way you came from, set that as the value
                if direction == reverse_dir[next_dir]:
                    directions[direction] = current_room
                # else set it as a '?'
                else:
                    directions[direction] = '?'
            # add the new room id's direction
            visited[new_room.id] = directions
        else:
            visited[new_room.id][reverse_dir[next_dir]] = current_room
        #from directions available - pick a new one again.
        next_dir = pickRandomDirection(new_room, visited)
        #set current_room as the new room you're in.
        current_room = new_room
    # return full path
    return path

def find_next_room(visited):
    ...
    while queue.size() > 0:
        ...
        path = queue.dequeue()
        room = path[-1][1]
        #pick a random direcction
        new_dir = pickRandomDirection(room, visited)
        # if new dir is not none, traverse.
        if new_dir is not None:
            traversal_path = []
            for pair in path:
                if pair[0] is not None:
                    traversal_path.append(pair[0])
            return (traversal_path, path[-1][1])
...
while current_room is not None:
        traversalPath += get_to_deadend(current_room, visited)
